...
import { isEidsPublicListingLocked } from "@/lib/kernel/compliance/circuit-breakers";
...
export function StallForm() {
  ...
  const eidsLocked = isEidsPublicListingLocked();
  const modelNotice =
    ...
    <form onSubmit={onSubmit} className="space-y-3">
      ...
      <label className="block text-sm">
        ...
        >
          ...
          <option value="REAL_ESTATE" disabled={eidsLocked}>
            {copy.realEstateOption}
          </option>
          <option value="VEHICLE" disabled={eidsLocked}>
            {copy.vehicleOption}
          </option>
        </select>
      </label>
      ...
      <Button type="submit" disabled={pending || (eidsLocked && isAssetCategory(category))}>

...
import {
  EIDS_PUBLIC_LISTING_LOCKED_ERROR,
...
export function assertEidsPublicListingAllowed(category: MarketplaceProductCategory): void {
  if (isAssetCategory(category) && isEidsPublicListingLocked()) {
    throw new ForbiddenError(EIDS_PUBLIC_LISTING_LOCKED_ERROR);
  }
}

export function isPubliclyListableProduct(
  product: Pick<MarketplaceProductRecord, "status" | "category">,
): boolean {
  if (product.status !== "LISTED") {
    return false;
  }
  if (isAssetCategory(product.category) && isEidsPublicListingLocked()) {
    return false;
  }
  return true;
}

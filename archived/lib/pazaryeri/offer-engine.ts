...
import { withPazaryeriMoney, type PazaryeriEnginePorts } from "@/lib/pazaryeri/engine";
import {
...
export async function decideMarketplaceOffer(
  ...
  return withPazaryeriMoney(ports, async (tx) => {
    const racedOrder = await tx.pazaryeri.getOrderByBuyerAndProduct(offer.userId, product.id);
    if (racedOrder) {
      throw new Error("Bu alıcı için sipariş zaten var.");
    }
    const currentOffer = await tx.pazaryeri.getOffer(offer.id);
    if (!currentOffer || currentOffer.status !== "OPEN") {
      if (currentOffer?.status === "ACCEPTED") {
        const existing = currentOffer.orderId ? await tx.pazaryeri.getOrder(currentOffer.orderId) : null;
        return { applied: false, offer: currentOffer, order: existing };
      }
      throw new Error("Teklif kabul edilebilir durumda değil.");
    }

    const lock = await tx.locks.upsertLock({
      userId: offer.userId,
    ...
    const accepted = await tx.pazaryeri.updateOffer(offer.id, {
      status: "ACCEPTED",
      escrowHoldId: hold.id,
      orderId: order.id,
      updatedAt: now,
    });
    await tx.pazaryeri.rejectOpenOffersForProduct(product.id, offer.id, now);

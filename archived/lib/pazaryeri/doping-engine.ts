...
export async function purchaseMarketplaceDoping(
  ...
  return withPazaryeriMoney(ports, async (tx) => {
    const activeInTx = await tx.pazaryeri.getActiveDopingForProduct(product.id, now);
    if (activeInTx) {
      throw new Error("İlan zaten dopingli.");
    }
    ...
    const doping = await tx.pazaryeri.insertDoping({
      id: dopingId,
    ...
    const next = await tx.pazaryeri.updateProduct(product.id, {
      isDoped: true,

...
function sellerCreditKey(buyerId: string, productId: string): string {
  return `pazaryeri-purchase-seller:${buyerId}:${productId}`;
}

function platformCreditKey(buyerId: string, productId: string): string {
  return `pazaryeri-purchase-hold:${buyerId}:${productId}`;
}
...
export async function purchaseMarketplaceProduct(
  ...
  if (product.kind === "DIGITAL_GOOD") {
    ...
    await appendLedgerEntry(ports.ledger, {
      ...
      currencyCode: split.currencyCode,
      amountMinor: split.netMinor,
      direction: "CREDIT",
      label: "Yetkinİlan satıcı net",
      purpose: "pazaryeri-seller-net",
      idempotencyKey: sellerCreditKey(command.userId, product.id),
    });
    await appendLedgerEntry(ports.ledger, {
      userId: platformUserId,
      currencyCode: split.currencyCode,
      amountMinor: split.holdMinor,
      direction: "CREDIT",
      label: "Yetkinİlan platform hold",
      purpose: "pazaryeri-platform-hold",
      idempotencyKey: platformCreditKey(command.userId, product.id),
    });
...
export async function confirmMarketplaceDelivery(
  ...
  await releaseEscrowHold(
    { ledger: ports.ledger, escrow: ports.escrow },
    {
      referenceKey: pazaryeriOrderReferenceKey(order.id),
      payeeUserId: order.sellerUserId,
      platformUserId,
      now,
    },
  );

  const next = await ports.pazaryeri.updateOrder(order.id, {
    status: "DELIVERED",
    settledAt: now,
    deliveredAt: now,
    updatedAt: now,
...
export async function refundMarketplaceOrder(
  ...
  await refundEscrowHold(
    { ledger: ports.ledger, escrow: ports.escrow },
    { referenceKey: pazaryeriOrderReferenceKey(order.id), now },
  );

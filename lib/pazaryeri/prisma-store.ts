...
export function createPrismaPazaryeriStore(): PazaryeriStore {
  ...
  return {
    async insertProduct(product) {
      const row = await prisma.marketplaceProduct.create({
        data: {
          id: product.id,
    ...
    async getProduct(id) {
      const row = await prisma.marketplaceProduct.findUnique({ where: { id } });
      return row ? toProduct(row) : null;
    },
    async getProductBySlug(slug) {
      const row = await prisma.marketplaceProduct.findUnique({ where: { slug } });
      return row ? toProduct(row) : null;
    },
    ...
    async updateProduct(id, patch) {
      const row = await prisma.marketplaceProduct.update({
        where: { id },
        data: patch,
      });
      return toProduct(row);
    ...
    async getOrder(id) {
      const row = await prisma.marketplaceOrder.findUnique({ where: { id } });
      return row ? toOrder(row) : null;
    },
    async getOrderByEscrowHoldId(escrowHoldId) {
      const row = await prisma.marketplaceOrder.findUnique({ where: { escrowHoldId } });
      return row ? toOrder(row) : null;
    },
    async getOrderByBuyerAndProduct(userId, productId) {
      const row = await prisma.marketplaceOrder.findUnique({
    ...
    async insertOffer(offer) {
      const row = await prisma.marketplaceOffer.create({
        data: {
          id: offer.id,
    ...
    async getOffer(id) {
      const row = await prisma.marketplaceOffer.findUnique({ where: { id } });
    ...
    async updateOffer(id, patch) {
      const row = await prisma.marketplaceOffer.update({
        where: { id },
        data: patch,
      });
      return toOffer(row);
    ...
    async insertDoping(doping) {
      const row = await prisma.marketplaceDoping.create({
        data: {
          id: doping.id,
    ...
    async getActiveDopingForProduct(productId, now) {
      const row = await prisma.marketplaceDoping.findFirst({
    ...
    async pulseForUser(userId) {
      const [listedProducts, sold, bought, pendingDelivery, recentSold] = await Promise.all([
        prisma.marketplaceProduct.count({ where: { userId, status: "LISTED" } }),

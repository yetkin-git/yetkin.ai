...
export async function sealCorporateJobPosting(
  ...
  return ports.kurumsal.insertPosting({
    id: postingId,
...
export async function releaseCorporateJobPosting(
  ...
  await releaseEscrowHold(
    { ledger: ports.ledger, escrow: ports.escrow },
    {
      referenceKey: corporateJobReferenceKey(posting.id),
      payeeUserId: posting.awardedUserId,
      platformUserId: command.platformUserId ?? resolvePlatformTreasuryUserId(),
      now,
    },
  );

  return ports.kurumsal.updatePosting(posting.id, {
    status: "RELEASED",
    releasedAt: now,
    updatedAt: now,
  });

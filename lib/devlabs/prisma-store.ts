...
export function createPrismaDevLabsStore(): DevLabsStore {
  ...
  return {
    async insertProject(project) {
      const row = await prisma.devLabsProject.create({
        data: {
          id: project.id,
    ...
    async getProject(id) {
      const row = await prisma.devLabsProject.findUnique({ where: { id } });
      return row ? toProject(row) : null;
    },
    async listProjectsByOwner(userId) {
      const rows = await prisma.devLabsProject.findMany({
        where: { userId },
    ...
    async updateProject(id, patch) {
      const row = await prisma.devLabsProject.update({ where: { id }, data: patch });
      return toProject(row);
    },
    async insertApiKey(key) {
      const row = await prisma.devLabsApiKey.create({
        data: {
          id: key.id,
    ...
    async getApiKey(id) {
      const row = await prisma.devLabsApiKey.findUnique({ where: { id } });
      return row ? toKey(row) : null;
    },
    async listApiKeysForProject(projectId) {
      const rows = await prisma.devLabsApiKey.findMany({
        where: { projectId },
        orderBy: { createdAt: "desc" },
      });
    ...
    async updateApiKey(id, patch) {
      const row = await prisma.devLabsApiKey.update({ where: { id }, data: patch });
      return toKey(row);
    },
    async insertArtifact(artifact) {
      const row = await prisma.devLabsArtifact.create({
        data: {
          id: artifact.id,
    ...
    async getArtifact(id) {
      const row = await prisma.devLabsArtifact.findUnique({ where: { id } });
      return row ? toArtifact(row) : null;
    },
    async listArtifactsForProject(projectId) {
      const rows = await prisma.devLabsArtifact.findMany({
        where: { projectId },
        orderBy: { createdAt: "desc" },
      });
    ...
    async pulseForUser(userId) {
      const [projectsCount, activeKeysCount, revokedKeysCount, artifactsCount] = await Promise.all([
        prisma.devLabsProject.count({ where: { userId, status: "ACTIVE" } }),
        prisma.devLabsApiKey.count({ where: { userId, revokedAt: null } }),
        prisma.devLabsApiKey.count({ where: { userId, revokedAt: { not: null } } }),
        prisma.devLabsArtifact.count({ where: { userId } }),
      ]);

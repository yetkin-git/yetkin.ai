 ...
 *   npx tsx scripts/bake-office-ai-05-sealed-pack.ts --confirm-gemini-spend
 */
...
const HUNT_PROMPT =
  "Bu tablo ve metindeki sapmaları çapraz kontrol et; çelişen sayıları ve uydurma özet cümlelerini kırmızı ile işaretle ve nedenini yaz.";
...
function parseScriptPayload(raw: unknown): ScriptPayload {
  ...
  if (!/istisna|hata avı/iu.test(title)) {
    throw new Error("Senaryo başlığı İstisnalar / Hata Avı taşımalı.");
  }
  ...
  if (!/e-?posta/iu.test(bridge) || !/sıfırla/iu.test(bridge)) {
    throw new Error("GİRİŞ KÖPRÜSÜ 4. ders e-posta sıfırlama / otomasyon refleksini pekiştirmeli.");
  }
  if (!/halüsinasyon/iu.test(warmup) || !/güven/iu.test(warmup)) {
    throw new Error("Warm-up aşırı güven ve halüsinasyon stresini taşımalı.");
  }
  if (!/çapraz/iu.test(command) && !/hata avı/iu.test(command)) {
    throw new Error("Command beat çapraz kontrol veya hata avı komutunu istemeli.");
  }
  if (!/kırmızı/iu.test(command) || !/halüsinasyon/iu.test(command)) {
    throw new Error("Command beat kırmızı işaret ve halüsinasyon avı istemini taşımalı.");
  }
  if (/50\.450|doğrulanmış tablo|düzeltilmiş dedektif/iu.test(command)) {
    throw new Error("Spoiler yasağı: Command beat düzeltilmiş dedektif tablosunu göstermemeli.");
  }
  if (!/istisna/iu.test(prose) || !/hata avı/iu.test(prose)) {
    throw new Error("Senaryo İstisnalar ve Hata Avı taşımalı.");
  }
  if (
    !/Haftalık Sistem/u.test(last) ||
    !/30 Dakika/u.test(last) ||
    !/Sınav Köprüsü/u.test(last) ||
    !/6\. bölüm|altıncı bölüm/iu.test(last)
  ) {
    throw new Error("Gelecek Ders Köprüsü L6 Haftalık Sistem / 30 Dakika / Sınav Köprüsü / 6. bölüm taşımalı.");
  }
  if (/kirli/iu.test(prose)) {
    throw new Error("Vatandaş metninde «kirli» yok; Düzensiz Tablo veya Ham Veri kullan.");
  }
  if (!/54\.650/u.test(comparison) || !/halüsinasyon/iu.test(comparison)) {
    throw new Error("Beat 3 split-screen sol halüsinasyonlu tabloyu anlatmalı.");
  }
  if (!/formül/iu.test(pocket)) {
    throw new Error("CEBİNE KOY 1. adım toplamları formülle doğrulatmalı.");
  }
  if (!/mantık hatası/iu.test(pocket)) {
    throw new Error("CEBİNE KOY 2. adım mantık hatası sormalı.");
  }
  if (!/insan gözü/iu.test(pocket)) {
    throw new Error("CEBİNE KOY 3. adım kritik veriyi insan gözüyle kilitlemeli.");
  }
...
function writeSection5(script: ScriptPayload, exam: ExamPayload): void {
...
export const section5: Section = {
  ...
  pedagogicalObjective: "AI yanılınca formül, çapraz sorgu ve insan gözü kilidi ile halüsinasyonu yakalama rutinini göstermek.",
  contentMarkdown: ${JSON.stringify(`\n${markdown}`)},
};
`;
  writeUtf8("lib/academy/curricula/office_ai/section_5.ts", source);
}
...
const SCRIPT_PROMPT = `Sen yetkin.ai akademisinin yazar fırınısın (Gemini 3.8 Flash). İş Hayatında ve Ofiste Yapay Zekâ dersinin 5. bölümü (İstisnalar & Hata Avı: AI Yanılınca) için konuşma senaryosu yaz.
...
- Pekiştirme durakları:
  ...
  * CEBİNE KOY (Task öncesi, ~45 sn, 90–120 kelime, hedef 105, asla 80’in altında): Derste öğrenilen 3 somut adımı tane tane tekrarla. Adımları «1.» «2.» «3.» diye say. 1) Toplamları formülle doğrulat. 2) AI'a «Mantık hatası var mı?» diye sor. 3) Kritik veriyi insan gözüyle kilitlesin. Bu üç ifadeyi kelime olarak kullan: «formül», «mantık hatası», «insan gözü». En az altı tam cümle yaz.
- 4-beat reji:
...
- SEN dili. Anlatıcı Gözde. İkinci paragraf (ilk HOŞ GELDİN) TAM OLARAK "Selamlar, ben Gözde" ile başlar. Beşinci ders.
...
const EXAM_PROMPT = `Sen yetkin.ai akademi sınav yazarısın (Gemini 3.8 Flash). 01_office_ai 5. bölüm (istisnalar ve hata avı: toplamları formülle doğrulat, mantık hatası sor, kritik veriyi insan gözüyle kilitle) için ezberci olmayan 3 soruluk mini sınav yaz.
...
- Baraj 70. Ezber tanım yok; iş sahnesi (halüsinasyonlu 59.450 genel toplam, Yıldız Tekstil 21.500 sapması, satır toplamı-genel toplam çelişkisi, formülle doğrulama, insan gözü kilidi). «Kirli» kelimesi yok. E-posta / slayt bu dersin konusu değil.
- id değerleri birebir: q_off_l5_1, q_off_l5_2, q_off_l5_3.
...
async function copySealedAudioAlias(): Promise<void> {
  ...
  const dest = join(ROOT, "media-bake", "01_office_ai_05_audio.mp3");
  if (!existsSync(src)) {
    throw new Error(`Yayın MP3 yok: ${src}`);
  }
  mkdirSync(dirname(dest), { recursive: true });
  copyFileSync(src, dest);
  process.stdout.write("yazıldı media-bake/01_office_ai_05_audio.mp3\n");
}
...
async function main(): Promise<void> {
  ...
  writeUtf8("docs/curriculum/01_office_ai_05_script.md", renderCurriculumMarkdown(script));
  writeUtf8(
    "lib/academy/spoken-scripts/01_office_ai-5.md",
  ...
  if (args.skipExam) {
    ...
    exam = parseExamPayload(
      JSON.parse(readFileSync(join(ROOT, "docs", "curriculum", "01_office_ai_05_exam.json"), "utf8")),
    );
    ...
    writeUtf8(
      "docs/curriculum/01_office_ai_05_exam.json",
      `${JSON.stringify(exam, null, 2)}\n`,
    );
    writeUtf8(
      "lib/academy/lesson-exams/01_office_ai-5.json",
      `${JSON.stringify(exam, null, 2)}\n`,
    );
  }
  writeSection5(script, exam);
  ...
  process.stdout.write("01_office_ai bölüm 5 mühür paketi tamam.\n");
}

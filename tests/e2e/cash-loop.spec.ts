  ...
  test("canlı/sandbox HTTP nakit döngüsü yalnız E2E_CASH_SANDBOX=1 ile açılır", async ({
    ...
    const pulseBody = (await pulse.json()) as {
      ...
      data?: {
        pulse?: {
          wallet?: { live?: boolean };
          freelancer?: { live?: boolean };
          career?: { live?: boolean };
        };
      };
    };
    expect(pulseBody.ok).toBe(true);
    expect(pulseBody.data?.pulse?.wallet?.live).toBe(true);
    expect(pulseBody.data?.pulse?.freelancer?.live).toBe(true);
    expect(pulseBody.data?.pulse?.career?.live).toBe(true);
    ...
    if (topUp.status() === 200) {
      ...
      expect(body.data?.merchantOid).toBeTruthy();
    }
  });
});

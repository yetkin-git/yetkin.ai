...
describe("vatandaş kimlik UX yüzeyi", () => {
  ...
  it("kayıt formu güvenli şifre üretir ve kopyalandı bildirimi sunar", () => {
    ...
    expect(form).toContain("readSession");
    expect(form).toContain("readPostLoginPathFromSearch");
    expect(form).toContain("window.location.assign(");
    expect(form).toContain("register-resend");
    expect(form).toContain("AUTH_RESEND_CONFIRMATION_API_PATH");
    expect(copy).toContain("Tekrar Gönder");
    expect(copy).toContain("Doğrulama e-postası gönderilemedi. Tekrar gönder.");
    ...
    expect(page).toContain("nextPath={nextPath}");
    expect(page).toContain('variant="secondary"');
    expect(page).toContain("w-full");
    expect(page).toContain("h-dvh");
    expect(readSrc("components/auth/login-page-content.tsx")).toContain('variant="secondary"');
    expect(readSrc("components/auth/login-page-content.tsx")).toContain("w-full");

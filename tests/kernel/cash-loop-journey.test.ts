...
describe("T4 nakit döngüsü — PayTR clearing → emanet → vize", () => {
  it("sıfır bakiyeden CREDIT, emanet serbesti ve freelancer vizesini basar", async () => {
    ...
    expect(journey.visa.stamp.moduleId).toBe("freelancer");
    expect(journey.visa.stamp.sourceKind).toBe("FREELANCER_RELEASE");
    expect(journey.visa.portfolioItem.visaStampId).toBe(journey.visa.stamp.id);
  });
});

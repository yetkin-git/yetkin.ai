...
describe("akademi kurs tohumu yüzeyi", () => {
  ...
  it("ops SQL amiral kursu basar; hayalet SKU INSERT etmez", () => {
    ...
    expect(sql).not.toContain("ac_02_ecommerce_ai");
    expect(sql).not.toContain("ac_03_social_media_ai");
    expect(sql).not.toContain("ac_04_chatbot_nocode");
    expect(sql).not.toContain("ac_05_prompt_practice");
    expect(sql).toMatch(/id NOT IN \('ac_01_office_ai'\)/i);

...
const LESSON_KEY = "02_ecommerce_ai-2";
...
describe("02_ecommerce_ai-2 mühürlü ses", () => {
  it("spoken script ve dört sahne cue F.1.1 akışına kilitlenir; karaoke açıktır", () => {
    ...
    if (layer.kind === "article+karaoke") {
      expect(layer.audioSrc).toContain("/media/academy/audio/02_ecommerce_ai/02_ecommerce_ai-2.wav");
    ...
    expect(prose).toMatch(/mağazanın satış kaderini belirleyen o eşiği/u);
    expect(prose).toMatch(/Ben Aylin/u);
    expect(prose).toMatch(/Kendi ürününün dönüşüm odaklı açıklamasını görmeden üçüncü bölüme geçme/u);
    ...
    expect(prose).toMatch(/He bir/u);
    expect(prose).toMatch(/Me ta/u);
    ...
    expect(prose).not.toMatch(/\bH1\b/u);
    ...
    expect(cueScript).toMatch(/\bSEO\b/u);
    ...
    const section = readSrc("lib/academy/curricula/ecommerce_ai/section_2.ts");
    expect(section).not.toContain("cue-01");
    expect(readSrc("scripts/generate-academy-lesson-audio.ts")).toContain("02_ecommerce_ai-2");
  });

  it("göz katmanı dört SEO paneli kartını cue başlangıcına bağlar", () => {
  ...
  it("diskteki mühürlü WAV timings ile karaoke katmanını açar", () => {
    ...
    expect(layer.cues).toHaveLength(timings!.pieces.length);
  });
});

...
const LESSON_KEY = "02_ecommerce_ai-3";
...
describe("02_ecommerce_ai-3 mühürlü ses", () => {
  it("spoken script ve dört sahne cue F.1.1 akışına kilitlenir; karaoke açıktır", () => {
    ...
    if (layer.kind === "article+karaoke") {
      expect(layer.audioSrc).toContain("/media/academy/audio/02_ecommerce_ai/02_ecommerce_ai-3.wav");
    ...
    expect(prose).toMatch(/kargon müşteriye ulaştıktan sonra başlayan asıl savaşı/u);
    expect(prose).toMatch(/Ben Aylin/u);
    expect(prose).toMatch(/Kendi yorumlarının kök neden analizini görmeden dördüncü bölüme geçme/u);
    ...
    expect(cueScript).toMatch(/\bLED\b/u);
    ...
    const section = readSrc("lib/academy/curricula/ecommerce_ai/section_3.ts");
    expect(section).not.toContain("cue-01");
    expect(readSrc("scripts/generate-academy-lesson-audio.ts")).toContain("02_ecommerce_ai-3");
  });

  it("göz katmanı dört iade / soru-cevap paneli kartını cue başlangıcına bağlar", () => {

...
describe("Tek eğitmen DialogueTurn zaman çizelgesi", () => {
  ...
  it("Maya ve Ece Master Voice %100; Koray/Can tempo ayrı durmaz", () => {
    ...
    expect(academyDialogueSpeechRate("gozde", "google-ads-masterclass")).toBe(1);
    expect(academyDialogueSpeechRate("tarik", "meta-ads-masterclass")).toBe(1);
    expect(academyDialogueSpeechRate("gozde", "meta-ads-masterclass")).toBe(1);
    expect(academyDialogueSpeechRate("tarik", "eticaret-masterclass")).toBe(1);
    expect(academyDialogueSpeechRate("gozde", "eticaret-masterclass")).toBe(1);
    ...
    expect(academyDialogueReadingDurationSec("bir iki üç dört", "maya")).toBeCloseTo(
      (4 * ACADEMY_DIALOGUE_MS_PER_WORD) / 1000 / 1,
      5,
    );
  });

...
const LESSON_KEY = "02_ecommerce_ai-5";
...
describe("02_ecommerce_ai-5 mühürlü ses", () => {
  it("spoken script ve dört sahne cue F.1.1 akışına kilitlenir; karaoke açıktır", () => {
    ...
    if (layer.kind === "article+karaoke") {
      expect(layer.audioSrc).toContain("/media/academy/audio/02_ecommerce_ai/02_ecommerce_ai-5.wav");
    ...
    expect(prose).toMatch(/müşterinin parmağını durduran o ilk görseli/u);
    expect(prose).toMatch(/Ben Aylin/u);
    expect(prose).toMatch(/Kendi ürününün parmağı durduran kancasını görmeden altıncı bölüme geçme/u);
    ...
    expect(prose).toMatch(/Midcörni/u);
    expect(prose).toMatch(/Dal i/u);
    expect(prose).toMatch(/Kanva/u);
    expect(prose).toMatch(/Tik tok/u);
    expect(prose).toMatch(/İnstegram/u);
    ...
    expect(prose).not.toMatch(/\bDALL-E\b/u);
    expect(prose).not.toMatch(/\bCanva\b/u);
    expect(prose).not.toMatch(/\bTikTok\b/u);
    expect(prose).not.toMatch(/\bInstagram\b/u);
    ...
    expect(cueScript).toMatch(/\bMidjourney\b/u);
    expect(cueScript).toMatch(/\bDALL-E\b/u);
    expect(cueScript).toMatch(/\bCanva\b/u);
    expect(cueScript).toMatch(/\bTikTok\b/u);
    expect(cueScript).toMatch(/\bInstagram\b/u);
    ...
    expect(cueScript).not.toMatch(/Dal i/u);
    expect(cueScript).not.toMatch(/Kanva/u);
    expect(cues[0]?.text).toMatch(/fotoğrafa tıklamazsa/u);
    expect(cues[3]?.text).toMatch(/sıfır nokta sekiz/u);

    const fromCues = collapseAcademyLessonProse(cues.flatMap((cue) => cue.paragraphs ?? []).join(" "));
    const fromMd = collapseAcademyLessonProse(
      readSrc("lib/academy/spoken-scripts/02_ecommerce_ai-5.md")
        .replace(/<!--[\s\S]*?-->/gu, " ")
        .replace(/\s+/gu, " ")
        .trim(),
    );
    ...
    const section = readSrc("lib/academy/curricula/ecommerce_ai/section_5.ts");
    expect(section).not.toContain("cue-01");
    expect(readSrc("scripts/generate-academy-lesson-audio.ts")).toContain("02_ecommerce_ai-5");
  });

  it("göz katmanı dört görsel / sosyal vitrin kartını cue başlangıcına bağlar", () => {
  ...
  it("diskteki mühürlü WAV timings ile karaoke katmanını açar", () => {
    ...
    expect(timings!.durationSec).toBeGreaterThan(500);

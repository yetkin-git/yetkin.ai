...
describe("01_office_ai bölüm 4 — E-Posta Akışı Altın Şablon", () => {
  ...
  it("Beat 3 split-screen sol 142 okunmamış, sağ sıfırlanmış kutu; spoiler kapalı", () => {
    ...
    expect(loadAcademyLessonVisualStage(KEY)).toBeNull();
  ...
  it("sağ panel 3 grup kartı basar; 142 mail listesi yok", () => {
...
describe("01_office_ai bölüm 4 — senaryo ve mühür kapısı", () => {
  it("konuşma gövdesi arşivdedir; canlı yükleyici boş döner", () => {
    expect(isAcademySpokenScriptLessonKey(KEY)).toBe(false);
    expect(hasAcademyLessonCues(KEY)).toBe(false);
    expect(hasAcademyLessonVisualStage(KEY)).toBe(false);
    expect(loadAcademySpokenScriptMarkdownParagraphs(KEY)).toEqual([]);
    expect(loadAcademySpokenScriptProse(KEY)).toBe("");
    expect(existsSync(join(ROOT, "lib/academy/spoken-scripts/01_office_ai-4.md"))).toBe(false);
    const prose = readFileSync(join(ROOT, "archived/academy/01_office_ai-4/spoken-script.md"), "utf8");
    ...
    const archiveCues = readFileSync(join(ROOT, "archived/academy/01_office_ai-4/lesson-cues.json"), "utf8");
    expect(archiveCues).toContain("INBOX KAOSU");
    expect(loadAcademyLessonCues(KEY)).toEqual([]);
    const compare = academyVisualCompareStage(KEY, "cue-06");
    ...
    expect(academyExcelFocusZoomActive(KEY, 208.64)).toBe(false);
    expect(academyExcelMouseState(KEY, 208.69)?.visible).toBeFalsy();
  });
  ...
  it("ses mührü karaoke katmanını açar; mini sınav baraj 70 durur", () => {
    ...
    expect(dronAcademyPunchcardsForLesson(KEY)).toEqual([]);
    expect(isAcademyLessonAudioSealed(SLUG, KEY)).toBe(false);
  ...
  it("karaoke harf düşürmez; aktif kelime layout shift ve descender kesmez", () => {
    expect(loadAcademySealedAudioTimings(KEY)).toBeNull();
    expect(loadAcademyKaraokeStrip(KEY)).toEqual([]);
    const archive = JSON.parse(
      readFileSync(join(ROOT, "archived/academy/01_office_ai-4/lesson-audio-timings.json"), "utf8"),
    ) as { durationSec: number; pieces: { text: string }[] };
    expect(archive.durationSec).toBe(493.8);
    const stripText = archive.pieces.map((piece) => piece.text).join(" ");

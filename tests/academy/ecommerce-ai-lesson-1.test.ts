...
const LESSON_KEY = "02_ecommerce_ai-1";
...
describe("02_ecommerce_ai-1 mühürlü ses", () => {
  it("spoken script ve dört sahne cue F.1.1 akışına kilitlenir; karaoke açıktır", () => {
    ...
    expect(prose).toMatch(/mağazandaki o sessiz krizi/u);
    expect(prose).toMatch(/Ben Aylin/u);
    expect(prose).toMatch(/Kendi ürününün profesyonel listelemesini görmeden ikinci bölüme geçme/u);
    ...
    const fromMd = collapseAcademyLessonProse(
      readSrc("lib/academy/spoken-scripts/02_ecommerce_ai-1.md")
        .replace(/<!--[\s\S]*?-->/gu, " ")
        .replace(/\s+/gu, " ")
        .trim(),
    );
    ...
    const section = readSrc("lib/academy/curricula/ecommerce_ai/section_1.ts");
    expect(section).not.toContain("cue-01");
  });

  it("göz katmanı dört pazaryeri paneli kartını cue başlangıcına bağlar", () => {

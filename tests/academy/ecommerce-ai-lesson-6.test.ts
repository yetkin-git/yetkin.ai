...
const LESSON_KEY = "02_ecommerce_ai-6";
...
describe("02_ecommerce_ai-6 mühürlü ses", () => {
  it("spoken script ve dört sahne cue F.1.1 akışına kilitlenir; karaoke açıktır", () => {
    ...
    if (layer.kind === "article+karaoke") {
      expect(layer.audioSrc).toContain("/media/academy/audio/02_ecommerce_ai/02_ecommerce_ai-6.mp3");
    ...
    expect(words).toBeGreaterThanOrEqual(890);
    expect(words).toBeLessThanOrEqual(1600);
    expect(prose).toMatch(/kriz ve iade yönetimini/u);
    expect(prose).toMatch(/Ben Aylin/u);
    expect(prose).toMatch(/Otonom asistanı ilk gerçek görevinde çalıştırmadan bu eğitimi kapatma/u);
    ...
    expect(prose).toMatch(/Şopifay/u);
    expect(prose).toMatch(/Vatsap/u);
    expect(prose).toMatch(/Es i o/u);
    expect(prose).toMatch(/Ayda/u);
    ...
    expect(prose).not.toMatch(/\bShopify\b/u);
    expect(prose).not.toMatch(/\bWhatsApp\b/u);
    expect(prose).not.toMatch(/\bSEO\b/u);
    expect(prose).not.toMatch(/\bAIDA\b/u);
    ...
    expect(cueScript).toMatch(/\bShopify\b/u);
    expect(cueScript).toMatch(/\bWhatsApp\b/u);
    ...
    expect(cueScript).not.toMatch(/Şopifay/u);
    expect(cueScript).not.toMatch(/Vatsap/u);
    expect(cues[0]?.text).toMatch(/Satışlar patladığında/u);
    expect(cues[3]?.text).toMatch(/kriz felaket değil/u);

    const fromCues = collapseAcademyLessonProse(cues.flatMap((cue) => cue.paragraphs ?? []).join(" "));
    const fromMd = collapseAcademyLessonProse(
      readSrc("lib/academy/spoken-scripts/02_ecommerce_ai-6.md")
        .replace(/<!--[\s\S]*?-->/gu, " ")
        .replace(/\s+/gu, " ")
        .trim(),
    );
    ...
    const section = readSrc("lib/academy/curricula/ecommerce_ai/section_6.ts");
    expect(section).not.toContain("cue-01");
    expect(readSrc("scripts/generate-academy-lesson-audio.ts")).toContain("02_ecommerce_ai-6");
  });

  it("göz katmanı dört kriz / kapanış kartını cue başlangıcına bağlar", () => {
  ...
  it("diskteki mühürlü WAV timings ile karaoke katmanını açar", () => {
    ...
    expect(timings!.durationSec).toBeGreaterThan(480);

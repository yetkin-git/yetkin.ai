...
describe("akademi sınav kapısı (S58-A)", () => {
  it("satın alma sertifika basmaz; ≥70 SHA256 mühür ve kariyer vizesi basar", async () => {
    ...
    const result = await submitAcademyExam(ctx.ports, {
      courseId: ctx.course.id,
      userId: BUYER,
      answers: PASSING,
      now,
    ...
    expect(result.certificate?.curriculumSeal).toBe(academyCurriculumSealForSlug("rail-temel"));
  ...
  it("satın almadan sınav açılmaz; müfredat bitmeden sınav 403; ikinci geçiş aynı sertifikayı döner", async () => {
    const ctx = world();
    await ctx.ports.academy.insertCourse(ctx.course);
    await ctx.ports.academy.insertExam(ctx.exam);
    await expect(
      submitAcademyExam(ctx.ports, { courseId: ctx.course.id, userId: BUYER, answers: PASSING }),
    ...
    await completeAcademyCurriculum(ctx.ports, { courseId: ctx.course.id, userId: BUYER });
    const first = await submitAcademyExam(ctx.ports, {
      courseId: ctx.course.id,
      userId: BUYER,
      answers: PASSING,
    });
    const second = await submitAcademyExam(ctx.ports, {
      courseId: ctx.course.id,
      userId: BUYER,
      answers: PASSING,
    });
